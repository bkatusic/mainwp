<?php
/**
 * MainWP ReconnectSites Ability Tests
 *
 * Tests for the mainwp/reconnect-sites-v1 ability.
 *
...
/**
 * Tests for mainwp/reconnect-sites-v1 ability.
 *
 * @group abilities
 * @group abilities-sites
 */
class Test_ReconnectSites_Ability extends MainWP_Abilities_Test_Case {
    ...
    public function test_ability_is_registered() {
        ...
        $ability = wp_get_ability( 'mainwp/reconnect-sites-v1' );
        $this->assertNotNull( $ability, 'Ability mainwp/reconnect-sites-v1 should be registered.' );
    }
     ...
     * @return void
     */
    public function test_reconnect_sites_returns_expected_structure() {
        $this->skip_if_no_abilities_api();
        $this->set_current_user_as_admin();

        $site_id = $this->create_test_site( [
            'name' => 'Test Site',
            'url'  => 'https://test-reconnect-sites.example.com/',
        ] );

        $result = $this->execute_ability( 'mainwp/reconnect-sites-v1', [
        ...
        $this->assertIsArray( $result['reconnected'] );
     ...
     * @return void
     */
    public function test_reconnect_sites_requires_authentication() {
        $this->skip_if_no_abilities_api();
     ...
     * @return void
     */
    public function test_reconnect_sites_requires_manage_options() {
        $this->skip_if_no_abilities_api();
     ...
     * @return void
     */
    public function test_reconnect_sites_validates_input() {
        $this->skip_if_no_abilities_api();
        $this->set_current_user_as_admin();
     ...
     * @return void
     */
    public function test_reconnect_sites_queues_large_batch() {
        $this->skip_if_no_abilities_api();
        $this->set_current_user_as_admin();
        ...
        for ( $i = 1; $i <= 3; $i++ ) {
            $site_ids[] = $this->create_test_site( [
                ...
                'url'  => "https://batch-reconnect-{$i}.example.com/",
            ] );
        }
        ...
        $this->assertEquals( 'reconnect', $job_data['job_type'], 'Job type should be reconnect.' );
